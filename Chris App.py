...
def scenario_selector_analysis(data, column_mapping):
    """Perform the Scenario Selector analysis and prepare data for Excel output."""
    try:
        bid_price_col = column_mapping['Bid Price']
        bid_volume_col = column_mapping['Bid Volume']
        baseline_price_col = column_mapping['Baseline Price']
        ...
        incumbent_col = column_mapping['Incumbent']
        supplier_name_col = column_mapping['Supplier Name']
        grouping_col = column_mapping.get('Grouping', None)
        ...
        # Prepare the DataFrame for "Scenario Selector"
        # Group data by Bid ID to get unique entries
        scenario_selector_df = data.groupby(bid_id_col).first().reset_index()

        # Keep only necessary columns
        columns_to_keep = [bid_id_col, facility_col, incumbent_col, baseline_price_col, bid_volume_col, 'Baseline Spend']
        if grouping_col:
            columns_to_keep.append(grouping_col)

        scenario_selector_df = scenario_selector_df[columns_to_keep]

        # Rename columns to match expected names
        rename_dict = {
            bid_id_col: 'Bid ID',
            facility_col: 'Facility',
            incumbent_col: 'Incumbent',
            baseline_price_col: 'Baseline Price',
            bid_volume_col: 'Bid Volume',
        }
        if grouping_col:
            rename_dict[grouping_col] = 'Grouping'
        scenario_selector_df.rename(columns=rename_dict, inplace=True)

        # Add empty columns for calculations
        scenario_selector_df['Awarded Supplier'] = ''
        scenario_selector_df['Awarded Supplier Price'] = ''
        scenario_selector_df['Awarded Volume'] = ''
        scenario_selector_df['Awarded Supplier Capacity'] = ''
        scenario_selector_df['Awarded Supplier Spend'] = ''
        scenario_selector_df['Savings'] = ''  # Initialize as empty string

        # Reorder columns to have 'Savings' at the end
        columns_order = ['Bid ID', 'Facility', 'Grouping', 'Incumbent', 'Baseline Price', 'Bid Volume', 'Baseline Spend',
                         'Awarded Supplier', 'Awarded Supplier Price', 'Awarded Volume', 'Awarded Supplier Capacity',
                         'Awarded Supplier Spend', 'Savings']
        # Ensure all columns exist in the DataFrame
        columns_order = [col for col in columns_order if col in scenario_selector_df.columns]

        scenario_selector_df = scenario_selector_df[columns_order]

        return scenario_selector_df
    except Exception as e:
        st.error(f"Error during Scenario Selector analysis: {str(e)}")
        logger.error(f"Error during Scenario Selector analysis: {str(e)}")
        return None
            ...
            if st.button("Run Analysis"):
                with st.spinner("Running analysis..."):
                    ...
                    with pd.ExcelWriter(output, engine='openpyxl') as writer:
                        ...
                        if "Scenario Selector" in analyses_to_run:
                            scenario_selector_df = scenario_selector_analysis(st.session_state.merged_data, st.session_state.column_mapping)
                            if scenario_selector_df is not None:
                                ...
                                # Now, modify the Excel workbook to add the formulas and data validation
                                workbook = writer.book
                                scenario_sheet = workbook["Scenario Selector"]
                                reference_sheet = workbook["Customizable Reference"]
                        
                                # Get the max row numbers
                                max_row_scenario = scenario_sheet.max_row
                                max_row_reference = reference_sheet.max_row
                        
                                # Create dictionaries to map column names to letters
                                scenario_headers = [cell.value for cell in scenario_sheet[1]]
                                scenario_col_letter = {cell.value: cell.column_letter for cell in scenario_sheet[1]}
                                ...
                                # Adjust the "Customizable Reference" sheet as per the requirements
                                # 1. Leave an empty column after "Baseline Spend"
                                baseline_spend_col_index = None
                                for idx, cell in enumerate(reference_sheet[1], start=1):
                                    if cell.value == 'Baseline Spend':
                                        baseline_spend_col_index = idx
                                        break
                                if baseline_spend_col_index is None:
                                    st.error("Baseline Spend column not found in Customizable Reference sheet.")
                                    return
                        
                                # Insert an empty column after "Baseline Spend"
                                empty_col_index = baseline_spend_col_index + 1
                                reference_sheet.insert_cols(empty_col_index)
                        
                                # Update headers and column letters after insertion
                                reference_headers = [cell.value for cell in reference_sheet[1]]
                                reference_col_letter = {cell.value: cell.column_letter for cell in reference_sheet[1]}
                        
                                # 2. Add a "Bid IDs" column after the empty column
                                bid_ids_col_index = empty_col_index + 1
                                reference_sheet.insert_cols(bid_ids_col_index)
                                reference_sheet.cell(row=1, column=bid_ids_col_index, value='Bid IDs')
                        
                                # Get the list of unique Bid IDs from the "Scenario Selector" sheet
                                bid_ids = [scenario_sheet.cell(row=row, column=openpyxl.utils.column_index_from_string(scenario_col_letter['Bid ID'])).value for row in range(2, max_row_scenario + 1)]
                        
                                # Write the Bid IDs into the "Bid IDs" column, starting from row 2
                                for idx, bid_id in enumerate(bid_ids, start=2):
                                    reference_sheet.cell(row=idx, column=bid_ids_col_index, value=bid_id)
                        
                                # 3. Add the scenario columns ("Scenario 1" to "Scenario 5") to the right of "Bid IDs" column
                                num_scenarios = 5
                                scenario_names = [f'Scenario {i}' for i in range(1, num_scenarios + 1)]
                                scenario_col_start_index = bid_ids_col_index + 1
                                for i, scenario_name in enumerate(scenario_names):
                                    col_index = scenario_col_start_index + i
                                    reference_sheet.insert_cols(col_index)
                                    reference_sheet.cell(row=1, column=col_index, value=scenario_name)
                        
                                # Update headers and column letters after adding scenario columns
                                reference_headers = [cell.value for cell in reference_sheet[1]]
                                reference_col_letter = {cell.value: cell.column_letter for cell in reference_sheet[1]}
                        
                                # Set up the scenario selector cell in the "Scenario Selector" sheet
                                # Place the scenario selector cell two columns to the right of "Savings"
                        
                                # Find the index of "Savings" column
                                savings_col_index = None
                                for idx, cell in enumerate(scenario_sheet[1], start=1):
                                    if cell.value == 'Savings':
                                        savings_col_index = idx
                                        break
                                if savings_col_index is None:
                                    st.error("Savings column not found in Scenario Selector sheet.")
                                    return
                        
                                # Insert an empty column after "Savings" to leave a space
                                scenario_sheet.insert_cols(savings_col_index + 1)
                                scenario_headers = [cell.value for cell in scenario_sheet[1]]
                                scenario_col_letter = {cell.value: cell.column_letter for cell in scenario_sheet[1]}
                                savings_col_index += 1  # Update index due to insertion
                        
                                # Scenario selection cell
                                scenario_selection_col_index = savings_col_index + 1
                                scenario_selection_cell = f"{get_column_letter(scenario_selection_col_index)}1"
                                scenario_sheet.cell(row=1, column=scenario_selection_col_index, value=scenario_names[0])  # Default scenario
                        
                                # Set up data validation for scenario selection
                                dv = DataValidation(type="list", formula1=f'"{",".join(scenario_names)}"', allow_blank=False)
                                scenario_sheet.add_data_validation(dv)
                                dv.add(scenario_sheet[scenario_selection_cell])
                        
                                # Create a mapping of Bid IDs to their corresponding row indices in the "Customizable Reference" sheet
                                bid_id_to_row_indices = {}
                                for row in range(2, max_row_reference + 1):
                                    bid_id = reference_sheet[f"{reference_col_letter[st.session_state.column_mapping['Bid ID']]}{row}"].value
                                    if bid_id not in bid_id_to_row_indices:
                                        bid_id_to_row_indices[bid_id] = []
                                    bid_id_to_row_indices[bid_id].append(row)
                        
                                # Set formulas in the "Scenario Selector" sheet
                                # First, set the formula for "Awarded Supplier" column
                                awarded_supplier_col_letter = scenario_col_letter['Awarded Supplier']
                                bid_id_col_letter_scenario = scenario_col_letter['Bid ID']
                        
                                # In the "Customizable Reference" sheet, get the column letters
                                bid_ids_col_letter_ref = get_column_letter(bid_ids_col_index)
                                first_scenario_col_letter_ref = get_column_letter(scenario_col_start_index)
                                last_scenario_col_letter_ref = get_column_letter(scenario_col_start_index + num_scenarios - 1)
                                max_row_scenarios = max_row_reference
                        
                                # Define the ranges for formulas
                                scenario_range_ref = f"'Customizable Reference'!${first_scenario_col_letter_ref}$2:${last_scenario_col_letter_ref}${max_row_scenarios}"
                                scenario_headers_range_ref = f"'Customizable Reference'!${first_scenario_col_letter_ref}$1:${last_scenario_col_letter_ref}$1"
                                bid_ids_range_ref = f"'Customizable Reference'!${bid_ids_col_letter_ref}$2:${bid_ids_col_letter_ref}${max_row_scenarios}"
                        
                                for row in range(2, max_row_scenario + 1):
                                    bid_id_cell_scenario = f"{bid_id_col_letter_scenario}{row}"
                                    awarded_supplier_cell = f"{awarded_supplier_col_letter}{row}"
                        
                                    formula_awarded_supplier = (
                                        f"=INDEX({scenario_range_ref}, MATCH({bid_id_cell_scenario}, {bid_ids_range_ref}, 0), "
                                        f"MATCH({scenario_selection_cell}, {scenario_headers_range_ref}, 0))"
                                    )
                                    scenario_sheet[awarded_supplier_cell].value = formula_awarded_supplier
                        
                                # Now, set the formulas for other columns
                                for row in range(2, max_row_scenario + 1):
                                    bid_id = scenario_sheet[f"{scenario_col_letter['Bid ID']}{row}"].value
                                    bid_id_cell_scenario = f"{scenario_col_letter['Bid ID']}{row}"
                                    awarded_supplier_cell = f"{scenario_col_letter['Awarded Supplier']}{row}"
                        
                                    # Get the row indices for this Bid ID
                                    if bid_id in bid_id_to_row_indices:
                                        start_row = min(bid_id_to_row_indices[bid_id])
                                        end_row = max(bid_id_to_row_indices[bid_id])
                                    else:
                                        continue  # If Bid ID not found, skip
                        
                                    # Adjust ranges for this Bid ID
                                    bid_id_range_ref = f"'Customizable Reference'!${reference_col_letter[st.session_state.column_mapping['Bid ID']]}${start_row}:${reference_col_letter[st.session_state.column_mapping['Bid ID']]}${end_row}"
                                    supplier_name_range_ref = f"'Customizable Reference'!${reference_col_letter[st.session_state.column_mapping['Supplier Name']]}${start_row}:${reference_col_letter[st.session_state.column_mapping['Supplier Name']]}${end_row}"
                                    bid_price_range_ref = f"'Customizable Reference'!${reference_col_letter[st.session_state.column_mapping['Bid Price']]}${start_row}:${reference_col_letter[st.session_state.column_mapping['Bid Price']]}${end_row}"
                                    supplier_capacity_range_ref = f"'Customizable Reference'!${reference_col_letter[st.session_state.column_mapping['Supplier Capacity']]}${start_row}:${reference_col_letter[st.session_state.column_mapping['Supplier Capacity']]}${end_row}"
                        
                                    # Awarded Supplier Price using SUMIFS
                                    formula_price = (
                                        f"=IFERROR(SUMIFS({bid_price_range_ref}, "
                                        f"{bid_id_range_ref}, {bid_id_cell_scenario}, "
                                        f"{supplier_name_range_ref}, {awarded_supplier_cell}),\"\")"
                                    )
                                    scenario_sheet[f"{scenario_col_letter['Awarded Supplier Price']}{row}"].value = formula_price
                        
                                    # Awarded Volume (Supplier Capacity) using SUMIFS
                                    formula_volume = (
                                        f"=IFERROR(SUMIFS({supplier_capacity_range_ref}, "
                                        f"{bid_id_range_ref}, {bid_id_cell_scenario}, "
                                        f"{supplier_name_range_ref}, {awarded_supplier_cell}),\"\")"
                                    )
                                    scenario_sheet[f"{scenario_col_letter['Awarded Volume']}{row}"].value = formula_volume
                                    scenario_sheet[f"{scenario_col_letter['Awarded Supplier Capacity']}{row}"].value = formula_volume
                        
                                    # Awarded Supplier Spend
                                    formula_spend = (
                                        f"=IF({scenario_col_letter['Awarded Supplier Price']}{row}<>\"\", "
                                        f"{scenario_col_letter['Awarded Supplier Price']}{row}*{scenario_col_letter['Awarded Volume']}{row},\"\")"
                                    )
                                    scenario_sheet[f"{scenario_col_letter['Awarded Supplier Spend']}{row}"].value = formula_spend
                        
                                    # Savings
                                    formula_savings = (
                                        f"=IF({scenario_col_letter['Awarded Supplier Price']}{row}<>\"\", "
                                        f"({scenario_col_letter['Baseline Price']}{row}-{scenario_col_letter['Awarded Supplier Price']}{row})*{scenario_col_letter['Bid Volume']}{row},\"\")"
                                    )
                                    scenario_sheet[f"{scenario_col_letter['Savings']}{row}"].value = formula_savings
                        
                                    # Baseline Spend
                                    formula_baseline_spend = f"={scenario_col_letter['Baseline Price']}{row}*{scenario_col_letter['Bid Volume']}{row}"
                                    scenario_sheet[f"{scenario_col_letter['Baseline Spend']}{row}"].value = formula_baseline_spend
                                ...
                                for col_name in currency_columns_reference:
                                    ...
                                    if col_letter:
                                        for row_num in range(2, max_row_reference + 1):
                                            cell = reference_sheet[f"{col_letter}{row_num}"]
                                ...
                                currency_columns_scenario = ['Baseline Spend', 'Baseline Price', 'Awarded Supplier Price', 'Awarded Supplier Spend', 'Savings']
                                number_columns_scenario = ['Bid Volume', 'Awarded Volume', 'Awarded Supplier Capacity']
                        
                                for col_name in currency_columns_scenario:
                                    col_letter = scenario_col_letter.get(col_name)
                                    if col_letter:
                                        for row_num in range(2, max_row_scenario + 1):
                                            cell = scenario_sheet[f"{col_letter}{row_num}"]
                                            cell.number_format = '$#,##0.00'
                        
                                for col_name in number_columns_scenario:
                                    col_letter = scenario_col_letter.get(col_name)
                                    if col_letter:
                                        for row_num in range(2, max_row_scenario + 1):
                                            cell = scenario_sheet[f"{col_letter}{row_num}"]
                                ...
                                logger.info("Scenario Selector analysis completed.")
